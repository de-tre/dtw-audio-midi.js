...
window.onload = async ()=>{
    ...
    const inputFile = document.querySelectorAll("input[type=file]").forEach((e)=>e.addEventListener("click", (event)=>{
            ...
            if (elem.id == "inputaudio") selector = 0;
            else if (elem.id == "inputmidi") selector = 1;
            ...
            elem.onchange = async ()=>{
                ...
                else if (window.URL != undefined) try {
                ...
                if (elem.id == "inputaudio") features[0] = await analyseAudio(url);
                else if (elem.id == "inputmidi") {
                    if (fileObj.type == "audio/mid") features[1] = await analyseMIDI(fileObj);
                    else try {
    ...
    const btn = Array.from(document.getElementsByClassName("btn")).forEach((e)=>e.addEventListener("click", async (e)=>{
            ...
            // Download path_P.json
                ...
                if (!dtwData) console.log("Not available");
                else {
                    console.log("Saving...");
...
//# sourceMappingURL=index.757071bf.js.map

...
let costMatrix = async (N, M, R)=>{
    ...
    for(let i = 0; i < n; i++)// Set boundary conditions from said constraint region R = Sakoe-Chiba band
...
let computeAccumulatedCostMatrix = async (C, R, w)=>{
    ...
    for(let i = 1, len = R[0].findIndex((e)=>e > 0); i < len; i++)C[i][0] = C[i - 1][0] + C[i][0] * w["h"];
    for(let j = 1, len = R[1][0]; j < len; j++)C[0][j] = C[0][j - 1] + C[0][j] * w["v"];
    // Fill rest of the matrix w.r.t. R
    for(let i = 1; i < n; i++)for(let k = 1, len = R[1][i]; k < len; k++){
        let C_ik = C[i][k];
        C[i][k] = Math.min(C[i - 1][k] + w["h"] * C_ik, C[i][k - 1] + w["v"] * C_ik, C[i - 1][k - 1] + w["d"] * C_ik);
...
let computeOptimalWarpingPath = async (D)=>{
    ...
    while(n > 0 || m > 0){
        if (n == 0) cell = [
            ...
            let d_n1m = D[n - 1][m];
            let val = Math.min(d_n1m1, d_n1m, D[n][m - 1]);
            if (val == d_n1m1) cell = [
                n - 1,
                m - 1
            ];
            else if (val == d_n1m) cell = [
                n - 1,
                m
            ];
            else cell = [
...
//# sourceMappingURL=index.d1a826aa.js.map
